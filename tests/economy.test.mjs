...
import { createDefaultState, GENERATORS, costFor, costForAmount, buyGenerator, registerTap, passiveRate, milestoneMultiplier, prestigeMultiplier, prestigeGain, ascend, buyRelic, offlineReward, dailyStatus, claimDaily, rewardedAdStatus, grantRewardedBoost, adBoostMultiplier, canShowInterstitial, markInterstitialShown } from '../src/game.js';
...
test('prestige unlocks at 500k and materially accelerates the repeated opening',()=>{let s=createDefaultState(0);s.lifetimeLumens=499999;assert.equal(prestigeGain(s),0);s.lifetimeLumens=500000;assert.equal(prestigeGain(s),1);const r=ascend(s,1000);assert.equal(r.gain,1);assert.equal(r.state.totalStardust,1);assert.equal(r.state.generators.sparkDrone,0);assert.equal(r.state.lumens,15);assert.equal(r.state.pulseReadyAt,1000);assert.ok(prestigeMultiplier(r.state)>=1.35);});
